...
export const posting = (id: string, date: string, type: string, cashFlow: Transaction["cashFlow"], legs: AccountingLeg[]): Transaction => {
  const t = { id, date, type, cashFlow, legs }; assertBalanced(t); return t;
};
...
function applyTransaction(state: SimulationState, t: Transaction) {
  ...
  for (const leg of t.legs) {
    ...
    if (leg.type === "cash") {
      ...
      if (state.liabilities[leg.entityId].balance < 0n) throw new Error(`Negative liability balance in ${t.id}`);
    } else if (leg.type === "asset" && leg.entityId) {
      const p = state.positions[leg.entityId]; if (!p) throw new Error(`Unknown position in ${t.id}`);
      p.carryingCents += sign * leg.amount;
      if (leg.quantity !== undefined) p.quantity += sign * leg.quantity;
      if (p.quantity < 0n || p.carryingCents < 0n) throw new Error(`Negative position balance in ${t.id}`);
    }
  }
}
...
export class SemanticRunner {
  ...
  run(period: Period, events: KernelEvent[]): RunResult {
    ...
    for (const id of graph.topologicalOrder()) {
      const e = byId.get(id)!;
      if (!inPeriod(e.date, period)) throw new Error(`Event ${e.id} is outside period`);
      if (e.transaction.date !== e.date) throw new Error(`Transaction date mismatch for ${e.id}`);
      effects.push(e.effect); assertBalanced(e.transaction); transactions.push(e.transaction); applyTransaction(state, e.transaction);
    }
...
export function mortgagePrincipal(paymentCents: Money, interestCents: Money, balanceCents: Money): Money { return Math.min(paymentCents - interestCents, balanceCents); }

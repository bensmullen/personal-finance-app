...
const validatePrimitiveResume = (request: VerticalSlice4RunInput, loan: FixedAmortizingLoan): void => {
  ...
  const principalDue = claimTotal(claimsFor(request.openingState, loan.principalLiabilityId, "mortgage_principal_due"), request.input.baseCurrency);
  const interestDue = claimTotal(claimsFor(request.openingState, loan.interestPayableLiabilityId, "mortgage_interest_payable"), request.input.baseCurrency);
  ...
  const accrualState = rawAccrualState === undefined ? undefined : rawAccrualState.primitiveId === "P24" ? rawAccrualState : invalid("Accrual primitive state has the wrong primitive kind", `primitiveState.${loan.primitiveIds.accrual}`);
  const historicalClaims = principalDue.isPositive() || interestDue.isPositive(); const scheduledBeforeRun = contractualOccurrencesBefore(loan, request.runContext.simulationStart);
  if ((principal.compare(loan.originalPrincipal) < 0 || historicalClaims || scheduledBeforeRun > 0) && amortizationState === undefined) invalid("A resumed loan with prior contractual activity requires compatible P22 runtime state", `primitiveState.${loan.primitiveIds.amortization}`);
...
const validate = (request: VerticalSlice4RunInput, periods: readonly Period[]): void => {
  ...
  for (const loan of input.loans) {
    ...
    const principal = openingState.liabilities[loan.principalLiabilityId] ?? invalid(`Principal liability ${loan.principalLiabilityId} is missing`, `loans.${loan.id}.principalLiabilityId`); const payable = openingState.liabilities[loan.interestPayableLiabilityId] ?? invalid(`Interest-payable liability ${loan.interestPayableLiabilityId} is missing`, `loans.${loan.id}.interestPayableLiabilityId`);
    if (loan.principalLiabilityId === loan.interestPayableLiabilityId) invalid("Principal and interest payable require distinct liability state targets", `loans.${loan.id}.interestPayableLiabilityId`);
    if (!principal.balance.currency.equals(input.baseCurrency) || !payable.balance.currency.equals(input.baseCurrency) || !loan.originalPrincipal.currency.equals(input.baseCurrency)) invalid("Loan Money values must use the run base currency", `loans.${loan.id}`);
    if (loan.originalPrincipal.isNegative() || principal.balance.isNegative() || payable.balance.isNegative()) invalid("Loan opening balances cannot be negative", `loans.${loan.id}`);
    if (principal.balance.compare(loan.originalPrincipal) > 0) invalid("Current principal cannot exceed original principal when new draws are unsupported", `loans.${loan.id}.principalLiabilityId`, true);
    ...
    if (loan.ownerId !== input.ownerId && loan.ownerId !== input.householdId) invalid("Loan owner must be within the declared VS4 economic-owner scope", `loans.${loan.id}.ownerId`);
    for (const extra of loan.extraPrincipalPayments ?? []) { if (!extra.amount.isPositive() || !extra.amount.currency.equals(input.baseCurrency) || !extra.amount.amount.fitsScale(input.baseCurrency.minorUnitScale)) invalid("Extra principal must be positive posted Money", `loans.${loan.id}.extraPrincipalPayments.${extra.id}.amount`); if (!isContractualOccurrence(loan, extra.scheduledAt)) invalid("Extra principal must coincide with a contractual debt-service occurrence", `loans.${loan.id}.extraPrincipalPayments.${extra.id}.scheduledAt`); validatePolicy(extra.fundingPolicy, openingState, input.baseCurrency, `loans.${loan.id}.extraPrincipalPayments.${extra.id}.fundingPolicy`); }
    ...
    validatePrimitiveResume(request, loan);
  }

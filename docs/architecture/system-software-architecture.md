...
`HouseholdContentionPolicy` v1 is a mandatory, identity-bearing partial-order DAG over cross-domain operation classes. It has no implicit hierarchy. At a same sequencing instant, conservative shared-cash producer/consumer or consumer/consumer overlap requires an explicit dependency or a unique policy path; producer/producer overlap does not. Missing, cyclic, contradictory, or temporally impossible precedence is `invalid_model` when statically knowable; a later state-dependent failure rolls back its whole candidate period and is `incomplete`.

...
import { DependencyGraph, GoldenRunner, Rate, RateBasis, assertBalanced, domainId, fixedMortgagePayment, instant, mortgageInterest, mortgagePrincipal, month, money, posting, type AccountId, type KernelEvent, type LiabilityId, type PositionId, type SimulationState } from "../src/kernel.js";
...
describe("Semantic Kernel v0.1 golden scenarios", () => {
  ...
  it("S3 accrual creates a payable, and later settlement consumes it without new expense", () => {
    const initial = base({ [CHECKING]: acct(CHECKING, "checking", "10000") }, { [TAX]: { id: TAX, balance: money("0"), rate: Rate.fromPercentage("0", RateBasis.NominalAnnual) } });
  ...
  it("S4 uses one fixed payment for 12 months and separately tests reset/recast", () => {
    const sixPercent = Rate.fromPercentage("6", RateBasis.NominalAnnual);
    const sevenPercent = Rate.fromPercentage("7", RateBasis.NominalAnnual);
    const payment = fixedMortgagePayment(money("300000"), sixPercent, 360); expectMoney(payment, "1798.65");
    let balance = money("300000");
    for (let i = 0; i < 12; i++) { const interest = mortgageInterest(balance, sixPercent); const principal = mortgagePrincipal(payment, interest, balance); balance = balance.minus(principal); }
    expectMoney(balance, "296316.00");
    const resetInterest = mortgageInterest(balance, sevenPercent); expectMoney(resetInterest, "1728.51");
    expectMoney(fixedMortgagePayment(balance, sevenPercent, 348), "1991.63");
    expectMoney(money("1991.63").minus(resetInterest), "263.12");
    expectMoney(fixedMortgagePayment(money("10000"), Rate.fromPercentage("0", RateBasis.NominalAnnual), 12), "833.33");
    expect(() => fixedMortgagePayment(money("10000"), Rate.fromPercentage("6", RateBasis.Proportion), 12)).toThrow(/nominal annual basis/);
  });
...
describe("Kernel invariants", () => {
  ...
  it("enforces the half-open period and preserves the opening state on failure", () => {
    ...
    expect(() => new GoldenRunner(initial).run(month(2026, 1), bad)).toThrow(/outside period/); expect(JSON.stringify(initial)).toBe(before);
  });
});

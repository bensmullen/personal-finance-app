...
describe("Semantic Kernel v0.1 golden scenarios", () => {
  it("S1 salary → tax → retirement → expenses preserves gross recognition but only $2k cash increase", () => {
    ...
    expect(r.statements.income).toBe(money("10000")); expect(r.statements.expenses).toBe(money("6000")); r.transactions.forEach(expectTx);
  });
  ...
  it("S2 internal transfer is neutral, purchase moves cash into a position, mark-to-market adds no cash", () => {
    ...
    expect(r.statements.assets).toBe(money("110000")); expect(r.statements.netWorth).toBe(money("110000")); expect(r.statements.investingCashFlow).toBe(money("0")); expect(r.statements.assets).not.toBe(money("220000"));
  });
  ...
  it("S3 accrual creates payable, later settlement consumes it without new expense", () => {
    ...
    const jan = new GoldenRunner(initial).run(month(2026, 1), (s, e, t) => {
      s.liabilities.tax.balance = money("1000"); e.push({ id: "tax-recognition", kind: "recognition", amount: money("1000"), description: "expense: tax" });
  ...
  it("S4 fixed mortgage mechanics separate rate, payment, interest and principal; reset does not imply recast", () => {
    ...
    const interest = money("1500.00"); const principal = payment - interest; expect(principal).toBe(money("298.65")); expect(money("300000") - principal).toBe(money("299701.35"));
    let balance = 300000;
    for (let i = 0; i < 12; i++) { const p = fixedMortgagePayment(money(balance.toFixed(2)), 0.06, 360 - i); const interestCents = BigInt(Math.round(balance * 0.06 / 12 * 100)); balance -= Number(p - interestCents) / 100; }
    const recast = fixedMortgagePayment(money(balance.toFixed(2)), 0.07, 348); expect(recast).toBe(money("1991.63"));
    const resetInterest = money("1728.51"); expect(recast - resetInterest).toBe(money("263.12"));
    expect(payment - money("1748.26")).toBe(money("50.39"));
  });
  ...
  it("S5 mark-to-market then sale preserves $10k gain while investing cash flow is $110k", () => {
    ...
    expect(r.state.accounts.checking.cash).toBe(money("110000")); expect(r.state.positions.investment.quantity).toBe(0n); expect(r.statements.investingCashFlow).toBe(money("110000")); expect(r.statements.gains).toBe(money("10000")); expect(r.statements.netWorth).toBe(money("110000"));
  });
});
